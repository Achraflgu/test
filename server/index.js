...
async function addAdvancedBotBypass(args, strategy, attempt, hasRealCookies = false) {
  ...
  // Enable fake cookies automatically when no real cookies are available (for cookie-less bypass attempts)
  // Start using fake cookies from attempt 1 onwards (immediate bypass attempts)
  const shouldUseFakeCookies = process.env.USE_FAKE_COOKIES === 'true' || (!hasRealCookies && attempt >= 1);
  
  if (shouldUseFakeCookies) {
    try {
...
async function addYouTubeEnhancements(args, attempt = 0) {
  ...
  // Faster strategy rotation for better bypass (change every attempt instead of every 2)
  const strategy = attempt; // Change strategy every attempt for maximum diversity
  ...
  const addFreeProxy = () => {
    ...
    // Use proxy as-is (Oxylabs handles rotation internally, don't modify URL)
  ...
  // Check if we have real cookies (not auto-generated)
  let hasRealCookies = false;
  try {
    const cookieSetup = await setupYouTubeCookies();
    if (cookieSetup && cookieSetup.type === 'file') {
      const isAutoGenerated = cookieSetup.path && (
        cookieSetup.path.includes('.auto_generated_cookies') || 
        cookieSetup.path.includes('.temp_cookies')
      );
      hasRealCookies = !isAutoGenerated;
    } else if (cookieSetup && cookieSetup.type === 'browser') {
      hasRealCookies = true; // Browser cookies are real
    }
  } catch (err) {
  ...
  if (strategy === 0 || strategy === 1) {
    ...
    // Add advanced bot bypass methods (pass hasRealCookies so fake cookies can be enabled)
    addAdvancedBotBypass(args, 'Anti-Bot', attempt, hasRealCookies);
    
    addFreeProxy();
    
    console.log(`   🤖 Anti-bot client: ${client}`);
    // Log fake cookie status based on hasRealCookies and attempt
    if (!hasRealCookies && attempt >= 1) {
      console.log('   🔥 Fake cookies, session tokens, browser fingerprint enabled');
    } else {
      console.log('   🔥 Session tokens, browser fingerprint enabled');
    }
  ...
  args.push('--user-agent', userAgent);
    ...
    addAdvancedBotBypass(args, 'JSExecution', attempt, hasRealCookies);
  ...
  args.push('--user-agent', userAgent);
    ...
    // Add advanced bot bypass methods
    addAdvancedBotBypass(args, 'BrowserAutomation', attempt, hasRealCookies);
...
async function tryYoutubeDlExec(track, outputFolder, socket, downloadId, settings = {}) {
  try {
    ...
    // 🔥 CRITICAL FIX: Add cookies for YouTube authentication (skip auto-generated)
    let hasRealCookies = false;
    try {
      const cookieSetup = await setupYouTubeCookies();
      if (cookieSetup) {
        // Skip auto-generated cookies (they're fake and don't work)
        ...
        if (!isAutoGenerated) {
          ...
          console.log(`  ⚠️ Skipping auto-generated cookies (they don't work with YouTube bot detection)`);
        }
      } else {
        console.log(`  ⚠️ No real cookies available - will try fallback methods`);
      }
    } catch (err) {
      console.log(`  ⚠️ Cookie setup failed: ${err.message}`);
    }
    
    // Override config file player client for cookie-less downloads (web_embedded works better)
    if (!hasRealCookies) {
      console.log(`  🔄 No real cookies - overriding config to use web_embedded player client`);
      downloadOptions.extractorArgs = 'youtube:player_client=web_embedded';
    }
    
    console.log(`  🔧 Download options:`, JSON.stringify(downloadOptions, null, 2));
    
    // 🚀 OPTIMIZATION: For cookie-less downloads, try search method FIRST (works better than direct URL)
    let result;
    let useSearchMethod = false;
    
    if (!hasRealCookies && youtubeUrl && youtubeUrl.includes('youtube.com/watch')) {
      // Extract search query from track info
      ...
      // For real cookies or non-YouTube URLs, use direct method
      result = await youtubedl(youtubeUrl, downloadOptions);
    }
...
async function tryYtDlpFallback(tracks, outputFolder, outputTemplate, socket, downloadId, youtubeLinks = {}, settings = {}, attemptNumber = 0) {
  ...
  const downloadSingleTrack = async (track) => {
    ...
    // ====================================
    // Check if we have real cookies - if not, skip METHOD 1 faster (goes to METHOD 5 which has better bypass)
    let hasRealCookiesForMethod1 = false;
    try {
      const cookieSetup = await setupYouTubeCookies();
      if (cookieSetup) {
        const isAutoGenerated = cookieSetup.path && cookieSetup.path.includes('.auto_generated_cookies');
        hasRealCookiesForMethod1 = !isAutoGenerated;
      }
    } catch (err) {
      // No cookies available
    }
    ...
    // If no real cookies, only try once (then go to METHOD 5 with better bypass)
    const maxMethod1Attempts = hasRealCookiesForMethod1 ? 6 : 1;
    if (attemptNumber < maxMethod1Attempts) {
      console.log(`\n🎯 METHOD 1: Trying youtube-dl-exec (GitHub wrapper)...${hasRealCookiesForMethod1 ? '' : ' (no cookies, will skip to METHOD 5 if fails)'}`);
    ...
    if (track.url && track.url.includes('youtube.com') && attemptNumber < 4) {
      console.log(`\n🎯 METHOD 3: Trying Piped API (${PIPED_INSTANCES.length} instances)...`);
      try {
    ...
    if (youtubeLink && !youtubeLinks[`retry_${track.id}`]) {
      // Setup YouTube cookies first to check if we have real cookies
      let hasRealCookies = false;
      try {
        const cookieSetup = await setupYouTubeCookies();
        if (cookieSetup) {
      ...
      // For cookie-less downloads, try search method FIRST (bypasses bot detection)
      if (!hasRealCookies) {
        ...
        console.log(`\n🔧 Search Method Download (Attempt ${attemptNumber + 1})`);
        
        // Add enhanced methods for search method too
        await addYouTubeEnhancements(ytdlpArgs, attemptNumber);
        
        // Extra bypass for search downloads
        ...
        // Force metadata tags from known track fields
        ...
        // Real cookies available - use direct link (faster)
        console.log(`  🎯 Using direct YouTube link: ${youtubeLink}`);
        usingDirectLink = true;
      }
    }
    
    if (youtubeLink && !youtubeLinks[`retry_${track.id}`] && !useSearchMethod) {
      console.log(`  🎯 Using direct YouTube link: ${youtubeLink}`);
      usingDirectLink = true;
      ...
      // 🔥 CRITICAL FIX: Try cookies first (env var > file > browser), but skip auto-generated
      ...
      // For cookie-less downloads, prioritize web_embedded player client (works better)
      if (!hasRealCookies) {
        console.log(`  🔄 No real cookies - using web_embedded player client for better compatibility`);
        // Add web_embedded BEFORE addYouTubeEnhancements so it can override if needed
        const existingClientIndex = ytdlpArgs.findIndex(arg => arg === '--extractor-args');
        if (existingClientIndex === -1) {
          ytdlpArgs.splice(2, 0, '--extractor-args', 'youtube:player_client=web_embedded');
        } else {
          // Replace existing player_client if any
          const nextArgIndex = existingClientIndex + 1;
          if (nextArgIndex < ytdlpArgs.length) {
            ytdlpArgs[nextArgIndex] = ytdlpArgs[nextArgIndex].replace(/youtube:player_client=[^,]+/, 'youtube:player_client=web_embedded');
          }
        }
      }
      ...
      // Force web_embedded for search phase (works better for cookie-less downloads)
      // This is especially important when no real cookies are available
      ...
      if (clientArgIndex !== -1 && clientArgIndex + 1 < ytdlpArgs.length) {
        ...
        console.log(`🔍 Using web_embedded player client for better cookie-less compatibility`);
      } else if (!hasRealCookies) {
        // Add web_embedded if not present and no real cookies
        ytdlpArgs.splice(2, 0, '--extractor-args', 'youtube:player_client=web_embedded');
        console.log(`🔍 Added web_embedded player client (no real cookies available)`);
      }
      ...
      const result = await new Promise((resolve, reject) => {
        ...
        ytdlpProcess.on('close', async (code) => {
          if (code === 0) {
            ...
            if (fileExists) {
              const methodType = useSearchMethod ? 'SEARCH' : 'DIRECT';
              const displayName = useSearchMethod 
                ? (track.artist === 'Unknown Artist' ? track.name : `${track.artist} ${track.name}`).trim()
                : (track.artist === 'Unknown Artist' ? track.name : `${track.artist} - ${track.name}`);
              console.log(`✅ yt-dlp ${methodType} SUCCESS: ${displayName}`);
              successCount++;
              ...
              socket.emit('download:progress', {
                ...
                status: 'completed',
                progress: 100,
                message: `✅ Downloaded via yt-dlp ${methodType.toLowerCase()}: ${track.artist === 'Unknown Artist' ? track.name : `${track.artist} - ${track.name}`}`
              });
            ...
            const displayName = useSearchMethod 
              ? (track.artist === 'Unknown Artist' ? track.name : `${track.artist} ${track.name}`).trim()
              : (track.artist === 'Unknown Artist' ? track.name : `${track.artist} - ${track.name}`);
            console.log(`❌ yt-dlp ${methodType} FAILED: ${displayName} (exit code ${code})`);
            if (errorOutput) {
    ...
    if (allYouTube) {
      ...
      try {
        ...
        const failedTracksList = tracks.filter(track => {
          ...
          // Check if all methods failed due to bot detection
          let hasRealCookies = false;
          try {
            const cookieSetup = await setupYouTubeCookies();
            if (cookieSetup) {
          ...
          let failureMessage = `❌ Download failed - no tracks could be downloaded\nPlease try again or check the track URLs`;
          if (currentSuccess === 0 && !hasRealCookies) {
            failureMessage = `❌ Download failed - YouTube requires authentication\n\n🍪 **Cookies Required:**\nYouTube is blocking all cookie-less downloads.\n\n**Quick Fix (5 minutes):**\n1. Install "Get cookies.txt LOCALLY" extension\n2. Login to YouTube and export cookies\n3. Add to Railway: YOUTUBE_COOKIES variable\n\nSee server/cookies-setup.md for details`;
          }
