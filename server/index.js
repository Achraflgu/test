...
const allowedOrigins = [
  ...
  "https://test-s989.vercel.app",
  // Vercel preview deployments
  "https://test-s989-cex65hswg-achrafgu92-gmailcoms-projects.vercel.app",
  "https://test-s989-8uwefhham-achrafgu92-gmailcoms-projects.vercel.app",
  "https://test-s989-git-main-achrafgu92-gmailcoms-projects.vercel.app",
...
app.use(cors({
  origin: (origin, callback) => {
    ...
    // Check exact matches in allowedOrigins list
    ...
    if (origin.endsWith('.vercel.app') && origin.includes('achrafgu92')) {
      callback(null, true);
      return;
    }
    ...
    if (process.env.NODE_ENV === 'production') {
      ...
      console.log('⚠️ CORS allowed (dev mode) for:', origin);
      callback(null, true);
    }
  },
  credentials: true,
...
// Manual CORS headers as fallback for preflight requests
app.use((req, res, next) => {
  const origin = req.headers.origin;

  // Only set CORS headers for allowed origins
  if (origin && (allowedOrigins.includes(origin) ||
    (origin.endsWith('.vercel.app') && origin.includes('achrafgu92')))) {
...
// Rate limiting - strict for download endpoints
...
app.use('/api/download/start', downloadLimiter);
  ...
  try {
    ...
    if (isRedisAvailable()) {
      ...
      if (backupCookie && backupCookie.content) {
        ...
        // 🔧 FIX: Clear lock and start time since we're returning successfully
        activeRegenerations.delete(slotIndex);
        if (global['regenerationStartTimes']) {
          global['regenerationStartTimes'].delete(slotIndex);
        }
