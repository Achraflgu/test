...
async function addAdvancedBotBypass(args, strategy, attempt, hasRealCookies = false) {
  ...
  // Start using fake cookies from attempt 2 onwards (earlier attempts for better bypass)
  const shouldUseFakeCookies = process.env.USE_FAKE_COOKIES === 'true' || (!hasRealCookies && attempt >= 2);
...
async function addYouTubeEnhancements(args, attempt = 0) {
  ...
  const addFreeProxy = () => {
    ...
    // Enhanced: Add IP rotation and session parameters for better bypass
    if (process.env.OXYLABS_PROXY) {
      const baseProxy = process.env.OXYLABS_PROXY;
      // Add session rotation for better IP diversity
      const sessionId = Math.random().toString(36).substr(2, 8);
      const countryCodes = ['US', 'GB', 'DE', 'CA', 'FR', 'AU', 'JP', 'BR'];
      const randomCountry = countryCodes[Math.floor(Math.random() * countryCodes.length)];
      
      // Try to use country-specific endpoints if supported
      // Format: user-country-XX:pass@proxy.oxylabs.io:port
      if (baseProxy.includes('@')) {
        const [auth, rest] = baseProxy.split('@');
        const [host, port] = rest.split(':');
        // Add session for IP rotation
        proxy = `${auth}-session-${sessionId}-country-${randomCountry}@${host}:${port || '8000'}`;
      } else {
        proxy = baseProxy;
      }
      proxyType = 'Oxylabs Premium (Rotated)';
      console.log(`   🌐 Using Oxylabs proxy with IP rotation (${randomCountry}) to bypass YouTube blocking`);
    }
