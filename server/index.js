  ...
  try {
    ...
    try {
      ...
      if (cookieSetup) {
        ...
        // Skip auto-generated cookies (they don't work well with YouTube bot detection)
        if (!isAutoGenerated) {
          if (cookieSetup.type === 'file') {
            downloadOptions.cookies = cookieSetup.path;
            cookiesUsed = true;
            console.log(`  ✅ Authenticated with YouTube cookies (file) - for age-restricted videos`);
          } else if (cookieSetup.type === 'browser') {
            downloadOptions.cookiesFromBrowser = cookieSetup.browser;
            cookiesUsed = true;
            console.log(`  ✅ Authenticated with ${cookieSetup.browser} browser cookies - for age-restricted videos`);
          }
        } else {
          console.log(`  ⚠️ Skipping auto-generated cookies (they don't work with YouTube bot detection)`);
        }
      } else {
        console.log(`  ⚠️ No real cookies available - age-restricted videos may fail`);
      }
    } catch (err) {
    ...
    // 🔥 NEW: Check if failure was due to bot detection or age-restriction
    ...
    const hasAgeRestrictionError = errorMessage.includes('age-restricted') || 
                                   errorMessage.includes('LOGIN_REQUIRED');
    
    if (hasBotDetectionError || hasAgeRestrictionError) {
      console.log('  🚨 Authentication required (bot detection or age-restriction) in youtube-dl-exec');
      
      // Check if we tried without cookies
      if (!cookiesUsed) {
        console.log('  ⚠️ Video requires authentication but no cookies were used - please set up YouTube cookies');
        console.log('  💡 See server/cookies-setup.md for instructions on how to add cookies');
      } else {
        console.log('  🔄 Cookies were used but still failed - regenerating cookies...');
