...
export const PlaylistInput = ({ onPlaylistLoaded, hasExistingData, existingPlaylistName, initialUrl, onSearchTextDetected, currentTracks = [], onAddTracksAndPlay, onCheckPlayingState }: PlaylistInputProps) => {
  ...
  const [showSingleTrackDialog, setShowSingleTrackDialog] = useState(false);
  const [pendingSingleTrack, setPendingSingleTrack] = useState<{ playlist: Playlist; track: Track } | null>(null);
  ...
  const fetchPlaylistData = async () => {
    ...
    try {
      ...
      saveUrlToHistory(normalized);
       ...
       // Check if it's a single track (not a playlist)
       const isSingleTrack = tracks.length === 1 || playlist.totalTracks === 1;
       
       if (isSingleTrack) {
         // Single track: Show dialog to add to tracklist
         const track = tracks[0];
         setPendingSingleTrack({ playlist, track });
         setShowSingleTrackDialog(true);
         setLoading(false);
       } else {
  ...
  const handleAddSingleTrack = (playNext: boolean = false) => {
    if (!pendingSingleTrack) return;
    
    const { track } = pendingSingleTrack;
    
    // Check if duplicate
    if (isTrackDuplicate(track.id)) {
      toast.warning('This track is already in your playlist', {
        description: `"${track.name}" is already added`,
        duration: 3000
      });
      return;
    }
    
    // Add track at position 1 (beginning)
    onPlaylistLoaded(
      { ...pendingSingleTrack.playlist, totalTracks: 1 },
      [track],
    ...
    if (playNext && onAddTracksAndPlay) {
      ...
      toast.success(`✨ Added "${track.name}" to tracklist`);
    }
    
    setShowSingleTrackDialog(false);
    setPendingSingleTrack(null);
    setUrl("");
  };
  ...
  return (
    <div className="relative group">
      ...
      {/* Single Track Dialog */}
      <AlertDialog open={showSingleTrackDialog} onOpenChange={setShowSingleTrackDialog}>
        <AlertDialogContent className="sm:max-w-md">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-base text-muted-foreground pt-2">
              {pendingSingleTrack && (
                <span>
                  Add <span className="font-semibold text-foreground">"{pendingSingleTrack.track.name}"</span> to your tracklist?
                </span>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
          ...
          {pendingSingleTrack && (
            <div className="py-4">
              <div className="flex items-start gap-4 p-4 bg-gradient-to-br from-primary/5 to-accent/5 border border-primary/20 rounded-xl">
                {/* Track Image */}
                <div className="flex-shrink-0">
                  {pendingSingleTrack.track.imageUrl ? (
                    <img
                      src={pendingSingleTrack.track.imageUrl}
                      alt={pendingSingleTrack.track.name}
                ...
                {/* Track Info */}
                <div className="flex-1 min-w-0">
                  <h3 className="text-lg font-bold line-clamp-2 mb-1">{pendingSingleTrack.track.name}</h3>
                  <p className="text-sm text-muted-foreground line-clamp-1 mb-1">{pendingSingleTrack.track.artist}</p>
                  {pendingSingleTrack.track.album && (
                    <p className="text-xs text-muted-foreground/70 line-clamp-1 mb-2">{pendingSingleTrack.track.album}</p>
                  )}
                  
                  <div className="flex items-center gap-3 text-xs text-muted-foreground">
                    <span className="flex items-center gap-1">
                      <Clock className="h-3 w-3" />
                      {formatTrackDuration(pendingSingleTrack.track.duration)}
                    </span>
                  </div>
                  ...
                  <div className="mt-3 flex items-center gap-2 flex-wrap">
                    <button
                      onClick={() => window.open(pendingSingleTrack.track.url || pendingSingleTrack.playlist.url, '_blank', 'noopener,noreferrer')}
                      className="text-xs text-primary hover:underline flex items-center gap-1 transition-colors"
                    >
                      <ExternalLink className="h-3 w-3" />
                      Open in new tab
              ...
              {/* Duplicate Warning */}
              {pendingSingleTrack && isTrackDuplicate(pendingSingleTrack.track.id) && (
                <div className="mt-3 p-3 bg-amber-500/10 border border-amber-500/30 rounded-lg flex items-center gap-2">
                  ...
                  <span className="text-sm text-amber-700 dark:text-amber-400">
                    This track is already in your tracklist
                  </span>
                </div>
              )}
            </div>
          ...
          <AlertDialogFooter className="gap-2">
            ...
            {pendingSingleTrack && !isTrackDuplicate(pendingSingleTrack.track.id) && (
              <>
                <AlertDialogAction
                  onClick={() => handleAddSingleTrack(false)}
                  className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl"
                >
                  <Plus className="w-4 h-4 mr-2" />
                  Add to Tracklist
                ...
                {onAddTracksAndPlay && (
                  <AlertDialogAction
                    onClick={() => handleAddSingleTrack(true)}
                    className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl"
                  >
