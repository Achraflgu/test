  ...
  const fetchPlaylistData = async () => {
    ...
    try {
      ...
      saveUrlToHistory(normalized);
       ...
       // Single tracks: Always auto-add without dialog (seamless experience)
       if (isSingleTrack) {
         // Check if duplicate
         ...
         // Add track (position 1 if existing data, replace if no data)
         onPlaylistLoaded(playlist, tracks, hasExistingData ? 'append' : 'replace');
         
         // Creative toast for single tracks
         toast.success(
           <div className="flex flex-col gap-3 p-1">
             <div className="flex items-center gap-3">
               <div className="w-14 h-14 rounded-xl overflow-hidden flex-shrink-0 shadow-lg ring-2 ring-primary/20">
                 {track.imageUrl ? (
               ...
               <div className="flex-1 min-w-0">
                 <div className="font-bold text-base line-clamp-1 mb-1">✨ Added to Playlist!</div>
                 <div className="font-semibold text-sm line-clamp-1 text-foreground">{track.name}</div>
                 <div className="text-xs text-muted-foreground line-clamp-1 mt-0.5">by {track.artist}</div>
               </div>
             </div>
             ...
             {onAddTracksAndPlay && (
               <div className="flex gap-2 pt-1">
                 <button 
                   ...
                   className="flex-1 text-xs px-4 py-2.5 bg-primary hover:bg-primary/90 text-primary-foreground rounded-lg transition-all flex items-center justify-center gap-2 font-semibold shadow-md hover:shadow-lg"
                 >
                   <Zap className="h-4 w-4" />
                   Play Now
                 </button>
                 <button 
                   onClick={() => window.open(track.url || playlist.url, '_blank', 'noopener,noreferrer')}
                   className="text-xs px-3 py-2.5 bg-secondary hover:bg-secondary/80 rounded-lg transition-colors flex items-center gap-1.5 border border-border"
                 >
